...
func (bb *ByBit) PlaceOrders(p cryptodb.Plan, activePair cryptodb.Pair, o []cryptodb.Order) (err error) {
	...
	log.Printf("Order size adjusted for step: %s", o[cryptodb.TypeEntry].Size.Div(activePair.Order.Step).Floor().Mul(activePair.Order.Step).String())
